...
| `internal/tools/builtin`         | `Read`, `Write` (atomic temp+rename), `Edit` (exact-match replace), `Grep` (parallel walker pool, `.gitignore`-aware, binary-skip via NUL sniff). | `read.go`, `write.go`, `edit.go`, `grep.go`, `gitignore_walker.go`, `gitignore/` (vendored go-git pattern matcher) |
