// Command dataagent is the dataagent server binary. It speaks MCP 2025-11-25
...
const usageText = `dataagent — local AI agent server (research build)

USAGE
  dataagent <command> [flags]

COMMANDS
  init      generate ~/.dataagent/{config.yaml,mcp.json,skills,agents} on first run
  serve     start the HTTP + MCP Streamable HTTP server
  version   print build version

GLOBAL FLAGS
  --config <path>      path to config.yaml (default: ~/.dataagent/config.yaml)
  --host <addr>        bind address (default: 127.0.0.1)
  --port <int>         bind port (default: 7821)
  --log-level <level>  debug|info|warn|error (default: info)

Run "dataagent <command> --help" for command-specific help.
`
