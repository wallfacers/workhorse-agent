...
// runInit creates ~/.dataagent and its expected children on first run. It
...
func runInit(args []string, stdout, stderr io.Writer) error {
	...
	root := filepath.Join(home, ".dataagent")
	...
	if exists(cfgPath) {
		fmt.Fprintf(stderr, "dataagent: %s already exists; refusing to overwrite\n", cfgPath)
		fmt.Fprintln(stderr, "delete or back up the existing file, then re-run `dataagent init`")
		return errExitUsage
	}
	...
	fmt.Fprintf(stdout, "\ndataagent: configured under %s\n", root)
	fmt.Fprintf(stdout, "  config: %s\n", cfgPath)
	fmt.Fprintf(stdout, "  mcp:    %s\n", mcpPath)
	fmt.Fprintf(stdout, "  state:  %s\n", dbPath)
	fmt.Fprintf(stdout, "next step: `dataagent serve`\n")
	return nil
}
...
func promptInit(out io.Writer, in io.Reader) (initAnswers, error) {
	...
	fmt.Fprintln(out, "dataagent init — answer a few questions, defaults in [brackets].")
	fmt.Fprintln(out)
...
func renderConfigYAML(a initAnswers) []byte {
	...
	sb.WriteString("# dataagent configuration. See specs/configuration/spec.md for the full schema.\n")
