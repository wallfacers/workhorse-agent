...
当前没有 Go 实现的本地 AI agent 服务器：能多轮对话、并行工具执行、并发派发子 agent、接 MCP、加载 Skills，对外暴露 WebSocket 全双工协议供任意客户端（CLI / Web UI / IDE 扩展）接入。这一版要补齐这个空白，以 Anthropic Claude Code 的架构为参考思想（不复制代码、命名、协议字段），用 Go 重新设计与实现，定位本地单用户多会话、可商用研究项目。

## What Changes

- 新建 Go 单进程服务 `dataagent`，绑 `127.0.0.1`，单二进制分发
- HTTP REST 提供会话 CRUD、取消、压缩、健康检查
- WebSocket 提供全双工事件流（10 种 Server→Client，5 种 Client→Server）
...
- 会话持久化：SQLite（modernc.org/sqlite，纯 Go）；events 表 append-only，支持断线重连按 `since_event_idx` 增量拉取
...
- `api-protocol`: HTTP REST 端点 + WebSocket 全双工消息协议 + 事件日志格式 + 断线重连
...
- **新增依赖**：~10 个直接依赖（chi、nhooyr.io/websocket、modernc.org/sqlite、yaml.v3、jsonschema、errgroup/semaphore、slog、ulid、shlex）
