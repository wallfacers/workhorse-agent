...
V2 计划增强：可插拔规则系统、调用 Haiku 做语义分类、shell parser 树形分析。MVP 接受这些限制是因为 data-agent 是本地单用户工具，安全模型基于"信任用户的 prompt"，DangerousCommandGuard 主要防"无心之失"而非"恶意攻击"。这一限制 SHALL 在 README 与 docs/protocol.md 中明示。
