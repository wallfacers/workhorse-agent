...
Bash 工具在执行 `exec.CommandContext` 前 SHALL 对继承的进程 env 应用过滤：

- 默认**移除**以下危险环境变量（注入 / 路径劫持类）：`LD_PRELOAD`、`LD_LIBRARY_PATH`、`LD_AUDIT`、`DYLD_INSERT_LIBRARIES`、`DYLD_LIBRARY_PATH`、`DYLD_FALLBACK_LIBRARY_PATH`、`DYLD_FORCE_FLAT_NAMESPACE`、`PYTHONPATH` 注入相关、`NODE_OPTIONS` 含 `--require` / `--import`
- 默认**保留**：`PATH`、`HOME`、`USER`、`SHELL`、`LANG`、`LC_*`、`TERM`、`TZ`、provider API key（仅子 session 继承）
- 会话级 `env` 字段（创建 session 时传入）SHALL **合并到**过滤后的 env，但 SHALL **不能**重新引入上述黑名单变量（即使用户显式设也拒绝，启动日志 warn）
