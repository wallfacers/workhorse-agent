...
- [ ] 2.4 `workhorse-agent serve`：装配所有模块、按 config.server.host:port 绑定、注册 SIGTERM/SIGINT；按 §9.10 graceful shutdown 流程退出
...
- [ ] 10.1 实现 `internal/coord/agenttype.go` 加载 `~/.workhorse-agent/agents/*.yaml`（启动 + Dispatch 调用前热扫描）
...
- [ ] 14.5 实际跑一遍端到端：`workhorse-agent init` → `workhorse-agent serve` → curl 创建会话 → `curl -X POST .../stream` 发 user_message → `curl -N .../stream` 观察 SSE 事件流
...
- [ ] 15.6 archive change：`openspec archive init-workhorse-agent-mvp`（将 specs 移到 `openspec/specs/`）
