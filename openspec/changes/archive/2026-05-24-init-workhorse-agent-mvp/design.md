...
`workhorse-agent` 是从零开始的本地 AI agent 服务端。当前没有现成的 Go 实现能同时满足：多轮对话 + 并行工具 + 多 agent 并发派发 + MCP + Skills + 统一传输协议 + 自定义 UI 解耦。OpenCode 是 TypeScript 写的；Claude Code 是 TypeScript CLI、闭源且无 server 模式。空白处需要填补。
...
- 默认继承父的 workdir / env / provider / model（可在 `~/.workhorse-agent/agents/<name>.yaml` 或 Dispatch 参数覆盖）
...
部署：
1. `go build` 出二进制
2. 用户跑 `workhorse-agent init` 生成 `~/.workhorse-agent/config.yaml`（API key、provider、端口）
3. `workhorse-agent serve` 启动
...
回滚：本地服务，删二进制 + `~/.workhorse-agent/` 即清空。
