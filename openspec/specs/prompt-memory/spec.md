...
static base段 and the `<environment>` block (组装顺序为 `base → environment →
memory`，见 agent-loop spec「System prompt 组装顺序优先静态前缀」), so that the
static cache prefix precedes the dynamic memory content. When both memory files
are empty, the variable expands to an empty string and the system prompt MUST
NOT contain any memory-related framing or headers.
...
- **THEN** the rendered system prompt contains the memory text within byte-stable
  ...
  same session), positioned after the base and environment segments
